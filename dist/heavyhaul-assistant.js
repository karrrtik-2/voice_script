...
(function() {
    ...
    function extractOrderIdFromUrl() {
        ...
        // Check for order_id in query parameters
        if (urlParams.has('order_id')) {
            return urlParams.get('order_id');
        }
        
        // Check for order ID in URL path (common pattern: /orders/ID-12345)
        const orderPathMatch = url.match(/\/orders?\/([A-Za-z0-9-]+)/);
        if (orderPathMatch && orderPathMatch[1]) {
            return orderPathMatch[1];
        }
        
        // Fallback to a default if needed or return null
        return null;
    }
    
    // Get order ID from URL
    const CURRENT_ORDER_ID = extractOrderIdFromUrl();
