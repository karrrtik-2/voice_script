        ...
        (function() {
            ...
            function initVoiceAssistant(elements) {
                ...
                // Use a composite key for session storage: orderID_browserID
                const browserID = browserIdInput.value;
                const orderID = orderIdInput.value;
                const sessionStorageKey = `heavyhaul_session_${orderID}_${browserID}`;
                
                let currentSessionId = localStorage.getItem(sessionStorageKey) || null;
                ...
                let messageFeedbackState = {}; // Stores { messageId: { rating: null/'good'/'bad', comment: null/'text', ratingSubmitted: false, commentSubmitted: false } }
    
                if (currentSessionId) sessionIdInput.value = currentSessionId;
                ...
                function addMessage(text, isUser, messageId = null) {
                    ...
                    if (!isUser && messageId) {
                        ...
                        // Initialize feedback state for this new message
                        messageFeedbackState[messageId] = { rating: null, comment: null, ratingSubmitted: false, commentSubmitted: false };
                    }
                ...
                async function handleFeedbackClick(event) {
                    ...
                    if (!messageId || !sessionId || !messageFeedbackState[messageId]) {
                        console.error("Feedback Error: Missing messageId, sessionId, or state.");
                        return;
                    }
    
                    // Prevent action if this specific feedback type has already been successfully submitted
                    if ((feedbackType === 'good' || feedbackType === 'bad') && messageFeedbackState[messageId].ratingSubmitted) {
                        console.log("Rating feedback already submitted for this message.");
                        return;
                    }
                    // Check comment separately - user might rate then comment
                    if (feedbackType === 'comment' && messageFeedbackState[messageId].commentSubmitted) {
                    ...
                    if (feedbackType === 'comment') {
                        ...
                        // Update state
                        messageFeedbackState[messageId].comment = userCommentText;
                        button.classList.add('selected'); // Visually select comment button
                        button.disabled = true; // Disable after successful comment prompt
                        messageFeedbackState[messageId].commentSubmitted = true; // Mark as submitted locally immediately
                    }
                    // --- Handle Rating Buttons ---
                    else if (feedbackType === 'good' || feedbackType === 'bad') {
                        ratingValue = feedbackType;
                        // Update state
                        messageFeedbackState[messageId].rating = ratingValue;
                        ...
                        messageFeedbackState[messageId].ratingSubmitted = true; // Mark as submitted locally immediately
                    }
                ...
                async function processCommand(command) {
                    ...
                    if (!orderId) {
                        ...
                        addMessage('Sorry, I need an order ID.', false);
                        statusDiv.classList.remove('heavyhaul-pulse');
                    ...
                    // Construct the request with browser fingerprint
                    const requestData = { 
                        message: command, 
                        order_id: orderId,
                    ...
                    try {
                        console.log('Sending to /chat:', JSON.stringify(requestData));
                        ...
                        if (!response.ok) {
                            try {
                                const error = await response.json();
                                throw new Error(error.error || `Chat API ${response.status}`);
                            } catch (err) {
                        ...
                        if (error.message !== 'No audio endpoint' && 
                            ...
                            addMessage(`Sorry, I encountered an error: ${error.message || error}`, false);
                ...
                if (initSpeechRecognition()) {
                    speakBtn.addEventListener('click', async (event) => {
                        ...
                        if (!CURRENT_ORDER_ID) {
                            statusDiv.textContent = 'Cannot start: No Order ID.';
                            return;
                        }
                ...
                addMessage("Hello! I'm your HeavyHaulGBT assistant. How can I help you with your order?", false);
